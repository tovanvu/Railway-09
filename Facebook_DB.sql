...
Delimiter $$
CREATE PROCEDURE p_delete_national ( IN In_National_Name NVARCHAR(50) )
BEGIN
DECLARE nation_id TINYINT;
SELECT n.National_id INTO nation_id FROM `national` n WHERE n.National_Name = In_National_Name;
...
Delimiter $$
CREATE FUNCTION f_SoNhanVien() RETURNS MEDIUMINT
BEGIN
DECLARE v_max_staff MEDIUMINT;
...
Delimiter $$
CREATE FUNCTION f_Infor_staff (f_Email VARCHAR(50)) RETURNS NVARCHAR(100)
BEGIN
...
END $$
Delimiter ;
SELECT f_Infor_staff('nguyenvand@gmail.com');
...
Old_Office_id SMALLINT UNSIGNED NOT NULL,
ChangeDate DATETIME
);
DROP TRIGGER IF EXISTS Trg_Log_Office;
DELIMITER $$
CREATE TRIGGER Trg_Log_Office
AFTER UPDATE ON `staff`
FOR EACH ROW
BEGIN
INSERT INTO `log_office` (Old_Email, Old_Office_id, ChangeDate)
VALUES                   (OLD.Email, OLD.Office_id, now()     );
END$$
DELIMITER ;
UPDATE staff SET Office_id = 3 WHERE Staff_id = 2;
